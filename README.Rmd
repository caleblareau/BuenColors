...
df <- data.frame(names = factor(names(jdb_color_maps), ordered = TRUE), 
                 color_maps = unname(jdb_color_maps))

ggplot(df, aes(x = names, y = -1, fill = names)) +
  geom_bar(stat = "identity") + pretty_plot(fontsize = 20) + 
  scale_fill_manual(values = as.character(df$jdb_color_maps)[order(df$names)]) + coord_flip() +
...
```{r, colormapPlotExample, fig.height = 3, echo = TRUE}
xy <- 1:7
...
```
**P.S.**-- it will return a white/blank color for features that are not found in the color map. 
